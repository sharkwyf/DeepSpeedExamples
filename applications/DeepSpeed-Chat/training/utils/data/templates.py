...
import random
def format_to_sentence(tokenizer, prompt_sentence, chosen_sentence, reject_sentence, end_of_conversation_token, loss_on_reject=False):
    ...
    if format.endswith('{pos}') and '{neg}' in format:
        ...
        prompt = tokenizer.encode(prompt_sentence)
        p1 = tokenizer.encode(p1, add_special_tokens=add_special_tokens)
        reject = tokenizer.encode(reject_sentence, add_special_tokens=add_special_tokens)
        p2 = tokenizer.encode(p2, add_special_tokens=add_special_tokens)
        chosen = tokenizer.encode(chosen_sentence + end_of_conversation_token, add_special_tokens=add_special_tokens)
        input_ids = prompt + p1 + reject + p2 + chosen
        loss_masks = [0] * len(prompt) + [0] * len(p1) + [loss_on_reject] * len(reject) + [0] * len(
            p2) + [1] * len(chosen)
    ...
    #     chosen = tokenizer.encode(chosen_sentence)
    #     p2 = tokenizer.encode(p2)
    #     reject = tokenizer.encode(reject_sentence + end_of_conversation_token)
    #     input_ids = prompt + p1 + chosen + p2 + reject
    #     loss_masks = [0] * len(prompt) + [0] * len(p1) + [1] * len(chosen) + [0] * len(
    #         p2) + [1] * len(reject)
        ...
        p1 = tokenizer.encode(p1, add_special_tokens=add_special_tokens)
        chosen = tokenizer.encode(chosen_sentence + end_of_conversation_token, add_special_tokens=add_special_tokens)
        input_ids = prompt + p1 + chosen
        loss_masks = [0] * len(prompt) + [0] * len(p1) + [1] * len(chosen)
    # elif format.endswith('{neg}'):
    #     p1 = format.split('{neg}')[0]
    #     prompt = tokenizer.encode(prompt_sentence)
    #     p1 = tokenizer.encode(p1)
    #     reject = tokenizer.encode(reject_sentence + end_of_conversation_token)
    #     input_ids = prompt + p1 + reject
    #     loss_masks = [0] * len(prompt) + [0] * len(p1) + [1] * len(reject)
    # elif format.endswith('{1st}'):
    #     p1 = format.split('{1st}')[0]
    #     p1 = p1.format(pos=chosen_sentence, neg=reject_sentence)
    #     p2 = "the first one."
    #     prompt = tokenizer.encode(prompt_sentence)
    #     p1 = tokenizer.encode(p1)
    #     p2 = tokenizer.encode(p2 + end_of_conversation_token)
    #     input_ids = prompt + p1 + p2
    #     loss_masks = [0] * len(prompt) + [0] * len(p1) + [1] * len(p2)
